...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed):
        ...
        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=LR_ACTOR, eps=1e-8)

        # Critic Network (w/ Target Network)
        self.critic_local = Critic(state_size, action_size, random_seed).to(device)
        self.critic_target = Critic(state_size, action_size, random_seed).to(device)
        self.critic_optimizer = optim.Adam(self.critic_local.parameters(), lr=LR_CRITIC, eps=1e-8, weight_decay=WEIGHT_DECAY)

        # optional experiment of initializing target networks as local networks
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        for agent in range(self.agents):
            ...
            # running twice as 2 different observations are added each time
            # experiences = self.memory.sample()
            # self.learn(experiences, GAMMA)
